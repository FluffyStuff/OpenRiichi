...
#endif
{
    ...
    private static bool tenpai_or_winning_hand(ArrayList<Tile> hand_in, bool tenpai_only)
    {
        ...
        if ((tenpai_only && hand_in.size == 13) || (!tenpai_only && hand_in.size == 14))
        {
            bool same = true;
            bool offset = false;
    ...
    public static bool has_yaku(Player player, Tile? ron_tile, bool last_piece, bool kan_piece)
    {
        ArrayList<Tile> tiles = new ArrayList<Tile>();
        ...
        if (Yaku.riichi(player.riichi_turn) != null ||
            Yaku.tsumo(player.open_hand) != null ||
            Yaku.tan_yao(tiles, player.calls) != null)
            return true;
