...
Game* game_construct (GType object_type) {
	...
	self = (Game*) g_type_create_instance (object_type);
	{
		...
		i = 0;
		{
			...
			while (TRUE) {
				...
				_tmp1_ = tile_new ((gfloat) 1, i, i / 4);
				t = _tmp1_;
	...
	game_shuffle_tiles (self->priv->tiles, self->priv->tiles_length1, 10000);
	{
		gint i;
		i = 0;
		{
			...
			while (TRUE) {
				...
				gee_abstract_collection_add ((GeeAbstractCollection*) self->priv->wall_tiles, self->priv->tiles[i]);
				rot = (gfloat) ((i / 34) * 90);
				...
				_tmp8_ = vector_new ((gfloat) 180, (gfloat) 0, rot);
				_tmp9_ = _tmp8_;
				tile_set_rotation (self->priv->tiles[i], _tmp9_);
				_vector_unref0 (_tmp9_);
				_tmp10_ = vector_new (x, y, z);
				_tmp11_ = _tmp10_;
				tile_set_position (self->priv->tiles[i], _tmp11_);
				_vector_unref0 (_tmp11_);
			}
		}
	}
	_tmp12_ = g_rand_int_range (game_rnd, (gint32) 2, (gint32) 12);
	wall_split = (gint) _tmp12_;
	starting_player = wall_split % 4;
	_tmp13_ = MAX (((starting_player * 34) + (wall_split * 2)) - 14, 0);
	self->priv->drawing_tile = _tmp13_;
	game_dead_wall_split (self, starting_player, wall_split);
	game_flip_dora (self);
	{
		gint i;
		i = 0;
		{
			...
			while (TRUE) {
				...
				_tmp16_ = _tmp15_;
				_tmp17_ = g_strconcat ("Player", _tmp16_, NULL);
				_tmp18_ = _tmp17_;
				_tmp19_ = player_new (i, _tmp18_);
				_tmp20_ = _tmp19_;
				_g_object_unref0 (self->priv->players[i]);
				self->priv->players[i] = _tmp20_;
				_g_free0 (_tmp18_);
				_g_free0 (_tmp16_);
			}
		}
	}
	player_set_computer_player (self->priv->players[0], FALSE);
	{
		gint i;
		i = 0;
		{
			...
			while (TRUE) {
				...
				p = _tmp22_;
				_tmp23_ = gee_collection_get_size ((GeeCollection*) self->priv->wall_tiles);
				self->priv->drawing_tile = self->priv->drawing_tile % _tmp23_;
				_tmp24_ = gee_abstract_list_get ((GeeAbstractList*) self->priv->wall_tiles, self->priv->drawing_tile);
				_tmp25_ = (Tile*) _tmp24_;
				player_add_tile (self->priv->players[p], _tmp25_);
				_g_object_unref0 (_tmp25_);
				_tmp26_ = gee_abstract_list_get ((GeeAbstractList*) self->priv->wall_tiles, self->priv->drawing_tile);
				_tmp27_ = (Tile*) _tmp26_;
				gee_abstract_collection_remove ((GeeAbstractCollection*) self->priv->wall_tiles, _tmp27_);
...
void game_process (Game* self, gdouble dt) {
	...
	if (self->priv->state == GAME_GAME_STATE_WAITING_CALLS) {
				...
				p = _tmp7_;
				{
					...
					if (_tmp8_) {
						...
						if (_tmp11_ == CALL_ACTION_CALL_ACTION_ENUM_OPEN_KAN) {
							...
							player_do_open_kan (p, _tmp13_, _tmp14_, self->priv->last_played_tile, self->priv->current_player);
							player_set_call_action (p, NULL);
							player_steal_tile (self->priv->players[self->priv->current_player], self->priv->last_played_tile);
							_tmp15_ = player_get_position (p);
							player = _tmp15_;
							self->priv->current_player = player;
							game_draw_dead_wall (self);
						} else {
							CallAction* _tmp16_ = NULL;
							CallActionCallActionEnum _tmp17_;
							_tmp16_ = player_get_call_action (p);
							_tmp17_ = call_action_get_action (_tmp16_);
							if (_tmp17_ == CALL_ACTION_CALL_ACTION_ENUM_PON) {
								CallAction* _tmp18_ = NULL;
								Tile** _tmp19_ = NULL;
								gint _tmp20_;
								gint _tmp21_;
								_tmp18_ = player_get_call_action (p);
								_tmp19_ = call_action_get_tiles (_tmp18_, &_tmp20_);
								player_do_pon (p, _tmp19_, _tmp20_, self->priv->last_played_tile, self->priv->current_player);
								player_set_call_action (p, NULL);
								player_steal_tile (self->priv->players[self->priv->current_player], self->priv->last_played_tile);
								_tmp21_ = player_get_position (p);
								player = _tmp21_;
							}
						}
					}
		...
		if (player == (-1)) {
			{
				...
				for (p_it = 0; p_it < self->priv->players_length1; p_it = p_it + 1) {
					Player* _tmp22_;
					Player* p;
					_tmp22_ = _g_object_ref0 (p_collection[p_it]);
					p = _tmp22_;
					{
						gboolean _tmp23_ = FALSE;
						CallAction* _tmp25_ = NULL;
						CallActionCallActionEnum _tmp26_;
						if (p == self->priv->players[self->priv->current_player]) {
							_tmp23_ = TRUE;
						} else {
							CallAction* _tmp24_ = NULL;
							_tmp24_ = player_get_call_action (p);
						...
						_tmp25_ = player_get_call_action (p);
						_tmp26_ = call_action_get_action (_tmp25_);
						if (_tmp26_ == CALL_ACTION_CALL_ACTION_ENUM_CHI) {
							CallAction* _tmp27_ = NULL;
							Tile** _tmp28_ = NULL;
							gint _tmp29_;
							gint _tmp30_;
							_tmp27_ = player_get_call_action (p);
							_tmp28_ = call_action_get_tiles (_tmp27_, &_tmp29_);
							player_do_chi (p, _tmp28_, _tmp29_, self->priv->last_played_tile);
							player_set_call_action (p, NULL);
							player_steal_tile (self->priv->players[self->priv->current_player], self->priv->last_played_tile);
							_tmp30_ = player_get_position (p);
							player = _tmp30_;
						}
						_g_object_unref0 (p);
					}
		...
		if (player == (-1)) {
			gint _tmp31_;
			_tmp31_ = gee_collection_get_size ((GeeCollection*) self->priv->wall_tiles);
			if ((_tmp31_ - self->priv->kan_count) <= 0) {
				self->priv->state = GAME_GAME_STATE_FINISHED;
				return;
			}
		...
		_tmp33_ = player_turn_decision (self->priv->players[self->priv->current_player]);
		if (_tmp33_) {
			gboolean _tmp34_;
			_tmp34_ = player_get_computer_player (self->priv->players[self->priv->current_player]);
			_tmp32_ = !_tmp34_;
		} else {
			_tmp32_ = FALSE;
		}
	...
	if (self->priv->state == GAME_GAME_STATE_WAITING_TURN) {
		PlayerPlayerState _tmp35_;
		_tmp35_ = player_get_state (self->priv->players[self->priv->current_player]);
		if (_tmp35_ == PLAYER_PLAYER_STATE_READY) {
			TurnAction* _tmp36_ = NULL;
			TurnActionTurnActionEnum _tmp37_;
			_tmp36_ = player_get_turn_action (self->priv->players[self->priv->current_player]);
			_tmp37_ = turn_action_get_action (_tmp36_);
			switch (_tmp37_) {
				case TURN_ACTION_TURN_ACTION_ENUM_DISCARD:
				{
					TurnAction* _tmp38_ = NULL;
					Tile* _tmp39_ = NULL;
					TurnAction* _tmp40_ = NULL;
					Tile* _tmp41_ = NULL;
					_tmp38_ = player_get_turn_action (self->priv->players[self->priv->current_player]);
					_tmp39_ = turn_action_get_discard_tile (_tmp38_);
					player_discard_tile (self->priv->players[self->priv->current_player], _tmp39_);
					_tmp40_ = player_get_turn_action (self->priv->players[self->priv->current_player]);
					_tmp41_ = turn_action_get_discard_tile (_tmp40_);
					game_discard_tile (self, _tmp41_);
					break;
				}
				case TURN_ACTION_TURN_ACTION_ENUM_CLOSED_KAN:
				{
					TurnAction* _tmp42_ = NULL;
					Tile** _tmp43_ = NULL;
					gint _tmp44_;
					_tmp42_ = player_get_turn_action (self->priv->players[self->priv->current_player]);
					_tmp43_ = turn_action_get_kan_tiles (_tmp42_, &_tmp44_);
					player_do_closed_kan (self->priv->players[self->priv->current_player], _tmp43_, _tmp44_);
					game_draw_dead_wall (self);
				...
				{
					TurnAction* _tmp45_ = NULL;
					Tile* _tmp46_ = NULL;
					TurnAction* _tmp47_ = NULL;
					Pon* _tmp48_ = NULL;
					_tmp45_ = player_get_turn_action (self->priv->players[self->priv->current_player]);
					_tmp46_ = turn_action_get_late_kan_tile (_tmp45_);
					_tmp47_ = player_get_turn_action (self->priv->players[self->priv->current_player]);
					_tmp48_ = turn_action_get_late_kan_pon (_tmp47_);
					player_do_late_kan (self->priv->players[self->priv->current_player], _tmp46_, _tmp48_);
					game_draw_dead_wall (self);
					player_turn_decision (self->priv->players[self->priv->current_player]);
					break;
				}
				...
				{
					TurnAction* _tmp49_ = NULL;
					TurnActionTurnActionEnum _tmp50_;
					TurnAction* _tmp51_ = NULL;
					Tile* _tmp52_ = NULL;
					TurnAction* _tmp53_ = NULL;
					Tile* _tmp54_ = NULL;
					_tmp49_ = player_get_turn_action (self->priv->players[self->priv->current_player]);
					_tmp50_ = turn_action_get_action (_tmp49_);
					_tmp51_ = player_get_turn_action (self->priv->players[self->priv->current_player]);
					_tmp52_ = turn_action_get_discard_tile (_tmp51_);
					player_do_riichi (self->priv->players[self->priv->current_player], _tmp50_ == TURN_ACTION_TURN_ACTION_ENUM_OPEN_RIICHI, self->priv->game_turn, _tmp52_);
					_tmp53_ = player_get_turn_action (self->priv->players[self->priv->current_player]);
					_tmp54_ = turn_action_get_discard_tile (_tmp53_);
					game_discard_tile (self, _tmp54_);
					break;
				}
	...
	self->priv->state = GAME_GAME_STATE_WAITING_CALLS;
	{
		...
		i = 0;
		{
			...
			while (TRUE) {
				...
				if (_tmp3_) {
					gboolean _tmp4_;
					_tmp4_ = player_get_computer_player (self->priv->players[i]);
					_tmp2_ = !_tmp4_;
				} else {
					_tmp2_ = FALSE;
				}
		...
		i = end - 1;
		{
			...
			while (TRUE) {
				...
				if (player == 0) {
					...
					Vector* _tmp3_ = NULL;
					gfloat _tmp4_;
					_tmp2_ = tile_get_position (self->priv->tiles[i]);
					_tmp3_ = tile_get_position (self->priv->tiles[i]);
					...
					if (player == 1) {
						...
						Vector* _tmp6_ = NULL;
						gfloat _tmp7_;
						_tmp5_ = tile_get_position (self->priv->tiles[i]);
						_tmp6_ = tile_get_position (self->priv->tiles[i]);
						...
						if (player == 2) {
							...
							Vector* _tmp9_ = NULL;
							gfloat _tmp10_;
							_tmp8_ = tile_get_position (self->priv->tiles[i]);
							_tmp9_ = tile_get_position (self->priv->tiles[i]);
							...
							Vector* _tmp12_ = NULL;
							gfloat _tmp13_;
							_tmp11_ = tile_get_position (self->priv->tiles[i]);
							_tmp12_ = tile_get_position (self->priv->tiles[i]);
							_tmp13_ = vector_get_y (_tmp12_);
				...
				if (a < 14) {
					gee_abstract_collection_add ((GeeAbstractCollection*) self->priv->dead_wall, self->priv->tiles[i]);
					gee_abstract_collection_remove ((GeeAbstractCollection*) self->priv->wall_tiles, self->priv->tiles[i]);
				}
				a++;
			}
		}
		...
		i = end - 1;
		{
			...
			while (TRUE) {
				...
				if (player == 0) {
					...
					Vector* _tmp16_ = NULL;
					gfloat _tmp17_;
					_tmp15_ = tile_get_position (self->priv->tiles[i]);
					_tmp16_ = tile_get_position (self->priv->tiles[i]);
					...
					if (player == 1) {
						...
						Vector* _tmp19_ = NULL;
						gfloat _tmp20_;
						_tmp18_ = tile_get_position (self->priv->tiles[i]);
						_tmp19_ = tile_get_position (self->priv->tiles[i]);
						_tmp20_ = vector_get_x (_tmp19_);
						...
						if (player == 2) {
							...
							Vector* _tmp22_ = NULL;
							gfloat _tmp23_;
							_tmp21_ = tile_get_position (self->priv->tiles[i]);
							_tmp22_ = tile_get_position (self->priv->tiles[i]);
							...
							Vector* _tmp25_ = NULL;
							gfloat _tmp26_;
							_tmp24_ = tile_get_position (self->priv->tiles[i]);
							_tmp25_ = tile_get_position (self->priv->tiles[i]);
...
static void game_shuffle_tiles (Tile** tiles, int tiles_length1, gint power) {
	{
		...
		i = 0;
		{
			...
			while (TRUE) {
				...
				_tmp1_ = g_rand_int_range (game_rnd, (gint32) 0, (gint32) (tiles_length1 - 1));
				r1 = (gint) _tmp1_;
				_tmp2_ = g_rand_int_range (game_rnd, (gint32) 0, (gint32) (tiles_length1 - 1));
				r2 = (gint) _tmp2_;
				_tmp3_ = _g_object_ref0 (tiles[r1]);
				t = _tmp3_;
				_tmp4_ = _g_object_ref0 (tiles[r2]);
				_tmp5_ = _tmp4_;
				_g_object_unref0 (tiles[r1]);
				tiles[r1] = _tmp5_;
				_tmp6_ = _g_object_ref0 (t);
				_tmp7_ = _tmp6_;
				_g_object_unref0 (tiles[r2]);
				tiles[r2] = _tmp7_;
				_g_object_unref0 (t);
			}
		}
...
static void game_toggle_interface (Game* self, gboolean show) {
	...
	_tmp1_ = _g_object_ref0 (self->priv->players[0]);
	p = _tmp1_;
	if (self->priv->state == GAME_GAME_STATE_WAITING_CALLS) {
		gboolean _tmp2_ = FALSE;
		gboolean _tmp3_;
...
void game_mouse_click (Game* self, gint x, gint y, guint color_id, gboolean mouse_state) {
	...
	Tile* t;
	g_return_if_fail (self != NULL);
	_tmp0_ = _g_object_ref0 (self->priv->players[0]);
	player = _tmp0_;
		...
		if (_tmp4_ == ((0 != _tmp4__label0) ? _tmp4__label0 : (_tmp4__label0 = g_quark_from_static_string ("Continue")))) {
			...
			switch (0) {
				default:
				{
					...
					if (_tmp10_) {
						CallAction* _tmp11_ = NULL;
						CallAction* _tmp12_;
						_tmp11_ = call_action_new_ron (self->priv->last_played_tile);
						_tmp12_ = _tmp11_;
						player_set_call_action (player, _tmp12_);
						_call_action_unref0 (_tmp12_);
						player_set_state (player, PLAYER_PLAYER_STATE_READY);
					}
					break;
				}
	...
	_tmp13_ = player_tile_press (player, color_id);
	t = _tmp13_;
	if (t == NULL) {
		_g_object_unref0 (t);
		_button_unref0 (button);
	...
	if (self->priv->state == GAME_GAME_STATE_WAITING_TURN) {
		...
		TurnAction* _tmp15_;
		_tmp14_ = turn_action_new_discard (t);
		_tmp15_ = _tmp14_;
		player_set_turn_action (player, _tmp15_);
		...
		if (self->priv->state == GAME_GAME_STATE_WAITING_CHI) {
			...
			Tile* _tmp17_ = NULL;
			Tile* tile;
			_tmp16_ = player_get_hand (player);
			_tmp17_ = logic_chi_combination (self->priv->last_played_tile, t, _tmp16_);
			tile = _tmp17_;
			if (tile != NULL) {
				Tile* _tmp18_;
				Tile* _tmp19_;
				...
				_tmp19_ = _g_object_ref0 (tile);
				_tmp20_ = g_new0 (Tile*, 2 + 1);
				_tmp20_[0] = _tmp18_;
				_tmp20_[1] = _tmp19_;
				_tmp21_ = _tmp20_;
				_tmp21__length1 = 2;
				_tmp22_ = call_action_new (CALL_ACTION_CALL_ACTION_ENUM_CHI, _tmp21_, 2);
				_tmp23_ = _tmp22_;
				player_set_call_action (player, _tmp23_);
				_call_action_unref0 (_tmp23_);
			...
			if (self->priv->state == GAME_GAME_STATE_WAITING_CLOSED_OR_LATE_KAN) {
				...
				gint count;
				{
					...
					gint _p_size;
					gint _p_index;
					_tmp24_ = player_get_pons (player);
					_tmp25_ = _g_object_ref0 (_tmp24_);
					_p_list = _tmp25_;
					_tmp26_ = gee_collection_get_size ((GeeCollection*) _p_list);
					_p_size = _tmp26_;
					_p_index = -1;
					while (TRUE) {
						...
						_tmp27_ = gee_abstract_list_get ((GeeAbstractList*) _p_list, _p_index);
						p = (Pon*) _tmp27_;
						_tmp28_ = tile_get_tile_type (t);
						_tmp29_ = call_get_tiles ((Call*) p, &_tmp30_);
						_tmp31_ = tile_get_tile_type (_tmp29_[0]);
						if (_tmp28_ == _tmp31_) {
							TurnAction* _tmp32_ = NULL;
							TurnAction* _tmp33_;
							_tmp32_ = turn_action_new_late_kan (t, p);
							_tmp33_ = _tmp32_;
							player_set_turn_action (player, _tmp33_);
							_turn_action_unref0 (_tmp33_);
							player_set_state (player, PLAYER_PLAYER_STATE_READY);
							game_toggle_interface (self, FALSE);
						}
						_call_unref0 (p);
				...
				count = 0;
				{
					...
					_tile_list = _tmp36_;
					_tmp37_ = gee_collection_get_size ((GeeCollection*) _tile_list);
					_tile_size = _tmp37_;
					_tile_index = -1;
					while (TRUE) {
						...
						_tmp38_ = gee_abstract_list_get ((GeeAbstractList*) _tile_list, _tile_index);
						tile = (Tile*) _tmp38_;
						_tmp39_ = tile_get_tile_type (t);
						_tmp40_ = tile_get_tile_type (tile);
						if (_tmp39_ == _tmp40_) {
							gint _tmp41_;
							Tile* _tmp42_;
							Tile* _tmp43_;
							_tmp41_ = count;
							count = _tmp41_ + 1;
							_tmp42_ = _g_object_ref0 (tile);
							_tmp43_ = _tmp42_;
							_g_object_unref0 (tiles[_tmp41_]);
							tiles[_tmp41_] = _tmp43_;
							if (count == 4) {
								TurnAction* _tmp44_ = NULL;
								TurnAction* _tmp45_;
								_tmp44_ = turn_action_new_closed_kan (tiles, tiles_length1);
								_tmp45_ = _tmp44_;
				...
				if (self->priv->state == GAME_GAME_STATE_WAITING_RIICHI_DISCARD) {
					...
					TurnAction* _tmp47_;
					_tmp46_ = turn_action_new_riichi (t);
					_tmp47_ = _tmp46_;
					player_set_turn_action (player, _tmp47_);
					_turn_action_unref0 (_tmp47_);
					player_set_state (player, PLAYER_PLAYER_STATE_READY);
					self->priv->state = GAME_GAME_STATE_WAITING_TURN;
					game_toggle_interface (self, FALSE);
				}
	...
	a = 0;
	{
		...
		while (TRUE) {
			...
			if (a == 3) {
				CallAction* _tmp11_ = NULL;
				CallAction* _tmp12_;
				_tmp11_ = call_action_new (CALL_ACTION_CALL_ACTION_ENUM_OPEN_KAN, tiles, tiles_length1);
				_tmp12_ = _tmp11_;
				player_set_call_action (player, _tmp12_);
				_call_action_unref0 (_tmp12_);
				player_set_state (player, PLAYER_PLAYER_STATE_READY);
...
static void game_do_closed_or_late_kan (Game* self) {
	...
	pon = NULL;
	{
		...
		while (TRUE) {
			...
			p = (Pon*) _tmp4_;
			{
				...
				_t_list = _tmp6_;
				_tmp7_ = gee_collection_get_size ((GeeCollection*) _t_list);
				_t_size = _tmp7_;
				_t_index = -1;
				while (TRUE) {
					...
					_tmp8_ = gee_abstract_list_get ((GeeAbstractList*) _t_list, _t_index);
					t = (Tile*) _tmp8_;
					_tmp9_ = tile_get_tile_type (t);
					_tmp10_ = call_get_tiles ((Call*) p, &_tmp11_);
					_tmp12_ = tile_get_tile_type (_tmp10_[0]);
					if (_tmp9_ == _tmp12_) {
						Tile* _tmp13_;
						Pon* _tmp14_;
						if (tile != NULL) {
						...
						_tmp13_ = _g_object_ref0 (t);
						_g_object_unref0 (tile);
						tile = _tmp13_;
						_tmp14_ = _call_ref0 (p);
						_call_unref0 (pon);
						pon = _tmp14_;
						_g_object_unref0 (t);
						break;
	...
	_tiles_size_ = 0;
	{
		...
		gint _t1_size;
		gint _t1_index;
		_tmp15_ = player_get_hand (player);
		_tmp16_ = _g_object_ref0 (_tmp15_);
		_t1_list = _tmp16_;
		_tmp17_ = gee_collection_get_size ((GeeCollection*) _t1_list);
		_t1_size = _tmp17_;
		_t1_index = -1;
		while (TRUE) {
			...
			_tmp18_ = gee_abstract_list_get ((GeeAbstractList*) _t1_list, _t1_index);
			t1 = (Tile*) _tmp18_;
			if (tiles != NULL) {
				gint _tmp20_;
				gint _tmp21_;
				_tmp20_ = tile_get_tile_type (tiles[0]);
				_tmp21_ = tile_get_tile_type (t1);
			...
			_temp_size_ = 4;
			{
				...
				gint _t2_size;
				gint _t2_index;
				_tmp23_ = player_get_hand (player);
				_tmp24_ = _g_object_ref0 (_tmp23_);
				_t2_list = _tmp24_;
				_tmp25_ = gee_collection_get_size ((GeeCollection*) _t2_list);
				_t2_size = _tmp25_;
				_t2_index = -1;
				while (TRUE) {
					gpointer _tmp26_ = NULL;
					...
					_tmp26_ = gee_abstract_list_get ((GeeAbstractList*) _t2_list, _t2_index);
					t2 = (Tile*) _tmp26_;
					_tmp27_ = tile_get_tile_type (t1);
					_tmp28_ = tile_get_tile_type (t2);
					if (_tmp27_ == _tmp28_) {
						gint _tmp29_;
						Tile* _tmp30_;
						Tile* _tmp31_;
						_tmp29_ = count;
						count = _tmp29_ + 1;
						_tmp30_ = _g_object_ref0 (t2);
						_tmp31_ = _tmp30_;
						_g_object_unref0 (temp[_tmp29_]);
						temp[_tmp29_] = _tmp31_;
						if (count == 4) {
							gboolean _tmp32_ = FALSE;
							Tile** _tmp33_;
							Tile** _tmp34_;
							...
							_tmp33_ = (_tmp34_ = temp, (_tmp34_ == NULL) ? ((gpointer) _tmp34_) : _vala_array_dup4 (_tmp34_, temp_length1));
							tiles = (_vala_array_free (tiles, tiles_length1, (GDestroyNotify) g_object_unref), NULL);
							tiles = _tmp33_;
	...
	if (tile != NULL) {
		TurnAction* _tmp35_ = NULL;
		TurnAction* _tmp36_;
		_tmp35_ = turn_action_new_late_kan (tile, pon);
		_tmp36_ = _tmp35_;
		player_set_turn_action (player, _tmp36_);
		_turn_action_unref0 (_tmp36_);
	} else {
		TurnAction* _tmp37_ = NULL;
		TurnAction* _tmp38_;
		_tmp37_ = turn_action_new_closed_kan (tiles, tiles_length1);
		_tmp38_ = _tmp37_;
		player_set_turn_action (player, _tmp38_);
		_turn_action_unref0 (_tmp38_);
	}
	player_set_state (player, PLAYER_PLAYER_STATE_READY);
	game_toggle_interface (self, FALSE);
	tiles = (_vala_array_free (tiles, tiles_length1, (GDestroyNotify) g_object_unref), NULL);
	...
	a = 0;
	{
		...
		while (TRUE) {
			...
			if (a == 2) {
				CallAction* _tmp11_ = NULL;
				CallAction* _tmp12_;
				_tmp11_ = call_action_new (CALL_ACTION_CALL_ACTION_ENUM_PON, tiles, tiles_length1);
				_tmp12_ = _tmp11_;
				player_set_call_action (player, _tmp12_);
				_call_action_unref0 (_tmp12_);
				player_set_state (player, PLAYER_PLAYER_STATE_READY);
...
static void game_do_chi (Game* self) {
	...
	_tmp4_ = call_action_new (CALL_ACTION_CALL_ACTION_ENUM_CHI, tiles, tiles_length1);
	_tmp5_ = _tmp4_;
	player_set_call_action (player, _tmp5_);
	_call_action_unref0 (_tmp5_);
	player_set_state (player, PLAYER_PLAYER_STATE_READY);
	game_toggle_interface (self, FALSE);
	tiles = (_vala_array_free (tiles, tiles_length1, (GDestroyNotify) g_object_unref), NULL);
	_g_object_unref0 (player);
