...
static void tile_set_id (Tile* self, guint8 value);
...
guint8* tile_tiles_to_ints (Tile** tiles, int tiles_length1, int* result_length1) {
	...
	gint _t_size_;
	guint8* _tmp2_;
	_tmp0_ = g_new0 (guint8, tiles_length1);
	t = _tmp0_;
	t_length1 = tiles_length1;
	_t_size_ = tiles_length1;
	{
		gint i;
		i = 0;
		{
			...
			while (TRUE) {
				...
				t[i] = tiles[i]->priv->_id;
			}
		}
	}
	_tmp2_ = t;
	*result_length1 = t_length1;
	result = _tmp2_;
	return result;
}
...
Tile** tile_ints_to_tiles (Tile** tiles, int tiles_length1, guint8* ids, int ids_length1, int* result_length1) {
	...
	gint _t_size_;
	Tile** _tmp4_;
	_tmp0_ = g_new0 (Tile*, ids_length1 + 1);
	t = _tmp0_;
	t_length1 = ids_length1;
	_t_size_ = ids_length1;
	{
		gint i;
		i = 0;
		{
			...
			while (TRUE) {
				...
				_tmp2_ = _g_object_ref0 (tiles[ids[i]]);
				_tmp3_ = _tmp2_;
				_g_object_unref0 (t[i]);
				t[i] = _tmp3_;
			}
		}
	}
...
static void tile_class_init (TileClass * klass) {
	...
	g_object_class_install_property (G_OBJECT_CLASS (klass), TILE_ID, g_param_spec_uchar ("id", "id", "id", 0, G_MAXUINT8, 0, G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB | G_PARAM_READABLE));
}
...
static void _vala_tile_get_property (GObject * object, guint property_id, GValue * value, GParamSpec * pspec) {
	...
	switch (property_id) {
		...
		break;
		case TILE_ID:
		g_value_set_uchar (value, tile_get_id (self));
		break;
		default:
...
static void _vala_tile_set_property (GObject * object, guint property_id, const GValue * value, GParamSpec * pspec) {
	...
	switch (property_id) {
		...
		break;
		case TILE_ID:
		tile_set_id (self, g_value_get_uchar (value));
		break;
		default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
		break;
