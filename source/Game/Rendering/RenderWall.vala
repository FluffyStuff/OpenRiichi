...
public class RenderWall
{
    ...
    public RenderWall(RenderTile[] tiles, Vec3 tile_size, Vec3 center, float offset, int starting_player, int split)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            Vec3 pos = Calculations.vec3_plus(Calculations.rotate_y({}, i, Vec3() { z = offset }), center);
    ...
    private class WallPart
    {
        ...
        private void order()
        {
            for (int i = 0; i < wall.size; i++)
            {
                ...
                tile.position = Calculations.vec3_plus(Calculations.rotate_y({}, rotation, pos), position);
