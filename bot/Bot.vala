...
#endif
{
    ...
    public static CallAction make_call(int bot_level, Player player, Tile discard_tile, bool can_chi)
    {
        ...
        // TODO: Fix winning...
        if (Logic.can_win_with(player.hand, discard_tile)/* && Logic.has_yaku(player.hand, player.calls, false, false)*/)
            return new CallAction.ron(discard_tile);
        else if (Logic.can_open_kan(discard_tile, player.hand)/* && false*/)
        {
        ...
        else if (Logic.can_pon(discard_tile, player.hand)/* && false*/)
        {
        ...
        else if (Logic.can_chi(discard_tile, player.hand)/* && false*/)
        {
            Tile? two_less = null;
            Tile? one_less = null;
            Tile? one_more = null;
    ...
    public static TurnAction? make_move(int bot_level, Player player)
    {
        ...
        if (Logic.winning_hand(player.hand) && Logic.has_yaku(player.hand, player.calls, false, false))
        ...
        if (tenpai_tiles.size != 0)
            return new TurnAction.riichi(tenpai_tiles[0]);
        else if (Logic.can_closed_kan(player.hand)/* && false*/)
        {
            Tile[] kan = new Tile[4];
            foreach (Tile t in player.hand)
            {
        ...
        else if (Logic.can_late_kan(player.hand, player.pons)/* && false*/)
        {
