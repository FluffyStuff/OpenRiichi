...
public class Game
{
    ...
    private Cursor default_cursor = new Cursor.from_system(SystemCursor.ARROW);
    private Cursor hover_cursor = new Cursor.from_system(SystemCursor.HAND);
            ...
            if (players[current_player].state == Player.PlayerState.READY)
            {
                switch (players[current_player].turn_action.action)
                {
                    ...
                    case TurnAction.TurnActionEnum.RIICHI:
                        players[current_player].do_riichi(players[current_player].turn_action.action == TurnAction.TurnActionEnum.OPEN_RIICHI);
                        discard_tile(players[current_player].turn_action.discard_tile);
                        break;
    ...
    {
        players[current_player].discard_tile(t);
        last_played_tile = t;
    ...
    public void mouse_click(int x, int y, uint color_id)
    {
        // TODO: Something
        Player player = players[0];

        Button.ButtonEnum button = ui.click(x, y, color_id);

        if (button != Button.ButtonEnum.NONE)
        {
            switch (button)
            {
            case Button.ButtonEnum.CONTINUE:
                do_continue();
                toggle_interface(false);
                break;
            case Button.ButtonEnum.PON:
                do_pon();
                toggle_interface(false);
                break;
            case Button.ButtonEnum.KAN:
                if (state == GameState.WAITING_CALLS)
                {
                    do_open_kan();
                    toggle_interface(false);
                ...
                break;
            case Button.ButtonEnum.CHI:
                do_chi();
                break;
            case Button.ButtonEnum.RIICHI:
                state = GameState.WAITING_RIICHI_DISCARD;
                break;
            case Button.ButtonEnum.TSUMO:
                if (Logic.has_yaku(player.hand, player.calls, wall_tiles.size - kan_count == 0, false))
                {
